...
import { getAnthropicClient, MODEL, countWords } from '@/lib/anthropic/client'
...
Return ONLY the rewritten opening line — no explanation, no quotes, no preamble.`,
  ...
  const fixedContent = rest ? `${rewrittenHook}\n\n${rest}` : rewrittenHook

  await supabase
    .from('free_form_drafts')
    .update({ content: fixedContent, word_count: countWords(fixedContent) })
    .eq('post_id', postId)
