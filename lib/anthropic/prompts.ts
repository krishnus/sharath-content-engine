...
export function buildGeneratePostPrompt(params: GeneratePostPromptParams): string {
  const formatInstructions: Record<PostFormat, string> = {
    ...
    carousel: `Write a LinkedIn carousel. 10–12 slides total. Format EACH slide EXACTLY as:\nSLIDE N | HEADLINE: [concept name only, 5–8 words, max 50 characters] | BODY: [1–2 lines max, ~100 characters — the single sharpest insight, cut to the bone]\n\nSlide 1 = Cover — hook title as headline, week theme as body.\nSlides 2–(N-1) = One concept per slide.\nSlide N (last) = Closing: HEADLINE = one compelling reflection question (e.g. "Which of these are you paying right now?"), BODY = omit.\n\nHARD RULES — never break these:\n- HEADLINE must be the concept name only. NEVER prefix it with a sequence number or category label (e.g. never write "Tax 3:" or "Insight 4:" inside the headline). The sequence badge is added separately by the template.\n- BODY: 1–2 lines maximum (~100 characters). The single most devastating insight — not context, not summary. One line that lands. Two short lines are acceptable only when they form a single punching thought.\n- No bullet points, no dashes, no numbered lists. Plain prose only.\n- No emojis anywhere.\n- Final slide must be a reflection question — never a generic follow prompt.`,
