...
export function buildGeneratePostPrompt(params: GeneratePostPromptParams): string {
  const formatInstructions: Record<PostFormat, string> = {
    ...
    carousel: `Write a LinkedIn carousel. 8–10 slides. Format EACH slide EXACTLY as:\nSLIDE N | HEADLINE: [title in 5–8 words, MAXIMUM 50 characters — must fit in one line] | BODY: [body text in 30–60 words, 3–8 bullet points or short lines]\nSlide 1 = bold hook (the title slide). Slides 2–8 = one actionable idea per slide. Slide 9 = key takeaway. Slide 10 = reflection question + follow prompt.\nCRITICAL: Headlines must be concise — 5 to 8 short words, under 50 characters. Never exceed 8 words in a headline.`,
