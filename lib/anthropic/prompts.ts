...
export function buildGeneratePostPrompt(params: GeneratePostPromptParams): string {
  const formatInstructions: Record<PostFormat, string> = {
    ...
    carousel: `Write a LinkedIn carousel. 10–12 slides total.\n\nFirst, on its own line, output the series label:\nSERIES_LABEL: [one uppercase word describing the content unit — e.g. TAX, STEP, INSIGHT, TRUTH, SHIFT, PRINCIPLE, MISTAKE, HABIT]\n\nThen format EACH slide EXACTLY as:\nSLIDE N | HEADLINE: [concept name only, 5–8 words, max 50 characters] | BODY: [1–2 lines max, ~100 characters — the single sharpest insight, cut to the bone]\n\nSlide 1 = Cover — hook title as headline, week theme as body.\nSlides 2–(N-1) = One concept per slide.\nSlide N (last) = Closing: HEADLINE = one compelling reflection question, BODY = omit.\n\nHARD RULES:\n- SERIES_LABEL must be singular, uppercase, one word. Choose the word that best names each unit (a tax, a step, an insight — not a generic "SLIDE").\n- HEADLINE must be the concept name only. NEVER prefix it with a number or the series label (e.g. never write "Tax 3:" or "Insight 4:" inside the headline text).\n- BODY: 1–2 lines maximum. The single most devastating insight — one line that lands.\n- No bullet points, no numbered lists, no emojis. Plain prose only.\n- Final slide must be a reflection question, not a generic follow prompt.`,
