...
export function buildGeneratePostPrompt(params: GeneratePostPromptParams): string {
  const formatInstructions: Record<PostFormat, string> = {
    ...
    carousel: `Write a LinkedIn carousel. 10–12 slides total. Format EACH slide EXACTLY as:\nSLIDE N | HEADLINE: [5–8 words, max 50 characters] | BODY: [plain prose only, max 3 lines / ~150 characters]\n\nSlide 1 = Cover — hook title as headline, week theme as body.\nSlides 2–(N-1) = One concept per slide. If an idea needs more than 3 lines to express, split it across two slides.\nSlide N (last) = Closing: HEADLINE = one compelling reflection question (e.g. "Which of these are you experiencing right now?"), BODY = omit.\n\nHARD RULES — never break these:\n- Maximum 3 lines of body copy per slide (~150 characters). Never compress — split instead.\n- No bullet points, no dashes, no numbered lists. Plain flowing prose only.\n- HEADLINE: maximum 8 words and 50 characters.\n- Final slide must end with a reflection question — never a generic follow prompt.\n- No emojis anywhere.`,
